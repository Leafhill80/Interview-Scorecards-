...
function SettingsModal({ onClose, notionToken, notionDbId, onSave }) {
  ...
  const handleSave = () => { onSave(token.trim(), dbId.trim()); onClose(); };
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#0F172A", fontFamily: "system-ui" }}>Notion Integration</div>
        ...
        <div style={{ padding: "20px 24px", fontFamily: "system-ui", display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ padding: "10px 14px", background: "#F0FDF4", borderRadius: 8, fontSize: 12, color: "#166534", display: "flex", gap: 8 }}>
            <span>🔒</span>
            <span>Saved to <strong>localStorage</strong> in your browser only — never sent anywhere except directly to the Notion API.</span>
          </div>
          <div>
            <label style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "#6B7280", marginBottom: 6 }}>NOTION INTEGRATION SECRET</label>
            <input type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder="secret_xxxxxxxxxxxxxxxxxxxx" style={inp} />
            <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>From notion.so/my-integrations → your integration → Internal Integration Secret</div>
          </div>
          <div>
            <label style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "#6B7280", marginBottom: 6 }}>DATABASE ID</label>
            <input type="text" value={dbId} onChange={(e) => setDbId(e.target.value)} placeholder="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" style={inp} />
            <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>From your Notion database URL: notion.so/your-workspace/<strong>THIS-PART</strong>?v=...</div>
          </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #F3F4F6", display: "flex", justifyContent: "flex-end", gap: 8, fontFamily: "system-ui" }}>
          ...
          <button onClick={handleSave} style={{ padding: "8px 20px", borderRadius: 8, border: "none", background: "#0F172A", color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Save Credentials</button>
        </div>
      </div>
    </div>
  );
...
function SummaryModal({ onClose, candidateName, role, interviewer, interviewDate, scores, notes, notionToken, notionDbId }) {
  ...
  const handlePushNotion = async () => {
    ...
    try {
      const page = await pushToNotion({ token: notionToken, databaseId: notionDbId, candidate: candidateName, role, interviewer, date: interviewDate, scores, notes });
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        {notionStatus && (
          <div style={{ margin: "8px 24px 0", padding: "10px 14px", background: notionStatus.startsWith("✓") ? "#F0FDF4" : notionStatus.startsWith("⚠") ? "#FEF3C7" : "#FEF2F2", borderRadius: 8, fontSize: 12, fontFamily: "system-ui", color: notionStatus.startsWith("✓") ? "#166534" : notionStatus.startsWith("⚠") ? "#92400E" : "#991B1B" }}>
            {notionStatus}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
          <pre style={{ fontFamily: "'Courier New', monospace", fontSize: 12, color: "#374151", background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 8, padding: 16, whiteSpace: "pre-wrap", wordBreak: "break-word", margin: 0, lineHeight: 1.7 }}>
            {generateText()}
          </pre>
        </div>
...
export default function App() {
  ...
  const handleSaveCredentials = (token, dbId) => {
    setNotionToken(token); setNotionDbId(dbId);
  ...
  return (
    <div style={{ fontFamily: "'Georgia', 'Times New Roman', serif", background: "#F8FAFC", minHeight: "100vh", padding: "32px 16px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <button onClick={() => setShowSettings(true)} style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${notionConfigured ? "#BBF7D0" : "#E5E7EB"}`, background: notionConfigured ? "#F0FDF4" : "white", fontSize: 13, fontFamily: "system-ui", fontWeight: 600, cursor: "pointer", color: notionConfigured ? "#166534" : "#374151" }}>
      ...
      {showSettings && <SettingsModal onClose={() => setShowSettings(false)} notionToken={notionToken} notionDbId={notionDbId} onSave={handleSaveCredentials} />}
      {showSummary && <SummaryModal onClose={() => setShowSummary(false)} candidateName={candidateName} role={role} interviewer={interviewer} interviewDate={interviewDate} scores={scores} notes={notes} notionToken={notionToken} notionDbId={notionDbId} />}
    </div>
  );
}
